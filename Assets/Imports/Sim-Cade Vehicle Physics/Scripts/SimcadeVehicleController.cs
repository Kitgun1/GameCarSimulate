...
namespace Ashsvp
{
    public class SimcadeVehicleController : MonoBehaviour
    {
        ...
        public GamplayInputActions InputActions;
        ...
        private void Update()
        {
            var moveVector2 = InputActions.Player.Move.ReadValue<Vector2>();
            var isJump = InputActions.Player.Jump.ReadValue<float>();
            if (CanDrive && CanAccelerate)
            {
                accelerationInput = moveVector2.y;
                steerInput = moveVector2.x;
                brakeInput = isJump;
            }
